#! /usr/bin/env python3
"""
cortex.distiller.core — Sync an Obsidian vault to agent-specific formats.
...
Usage via CLI:
  cortex distill                        # sync all enabled targets
  cortex distill --dry-run              # show changes without writing
  cortex distill --list                 # list all vault notes with tier
  cortex distill --show-config          # print resolved paths as JSON
  cortex distill --purge                # preview drained log/session deletions
  cortex distill --purge-apply          # delete drained logs/sessions + rebuild
  cortex distill --config /path/to.yaml # use a specific config file
"""
...
def read_vault_schema(vault_path: Path) -> int | None:
    ...
    for candidate in (vault_path / "_sync" / "distilled" / "memory.json",):
        if candidate.exists():
            try:
...
def validate_target_config(
    name: str, cfg: dict, required_keys: list[str]
) -> bool:
    """Return True if all required keys are present; print error otherwise."""
    for key in required_keys:
        if key not in cfg:
            print(
                f"  ERROR: target '{name}' is missing required config key "
                f"'{key}' — skipping."
            )
            return False
    return True
...
def strip_leading_h1(text: str) -> str:
    """Drop a note body's own leading `# Title` (redundant with distiller heading)."""
...
def strip_related_section(text: str) -> str:
    """Drop a trailing `## Related` section (vault nav; dead weight in distilled context)."""
    return re.sub(
        r"\n#+\s+Related\b.*\Z",
        "",
        text,
...
def scan_vault(
    vault_path: Path, skip_dirs: set[str] | None = None
) -> list[VaultNote]:
...
def excluded(
    note: VaultNote, exclude_tags: set[str], vault_only_types: set[str]
) -> bool:
...
def build_wiki_graph(notes: list[VaultNote]) -> dict:
    ...
    god_nodes = sorted(
        [
            {"id": nid, "degree": d}
            for nid, d in total_degree.items()
            if d > threshold
        ],
...
def sync_core_context(
    ...
    print(
        f"\n--- core-context: {len(eager)} eager notes "
        f"({', '.join(eager_tiers)}) ---"
    )
    ...
    parts: list[str] = [
        ...
        "",
        "Always-loaded memory. Heavy/specialised knowledge is loaded on demand "
        "via skills.",
        "",
    ]
    for n in eager:
...
def sync_skill_embeds(
    notes: list[VaultNote], cfg: dict, strip_links: bool, dry: bool
) -> list[str]:
    ...
    for skill, ns in sorted(by_skill.items()):
        ...
        parts: list[str] = [
            ...
            "",
            "Distilled memory for this skill. "
...
def update_opencode_instructions(
    opencode_cfg_path: Path, core_out: Path, dry: bool
) -> None:
    ...
    if dry:
        print(
            f"  [DRY] would {action} opencode instructions -> [core-context.md]"
        )
    else:
...
def purge_drained_logs(
    vault_path: Path, apply: bool
) -> list[VaultNote]:
    ...
    if not apply:
        print(
            ...
            "Re-run with --purge-apply to delete."
        )
    return drained
...
def run_check(vault_path: Path) -> None:
    ...
    print(
        f"Vault schema version   : "
        f"{vault_schema if vault_schema is not None else '(none — fresh)'}"
    )

    if vault_schema is None:
        print(
            "Status                 : OK — fresh vault; "
            "first distill will stamp schema."
        )
        return
    if vault_schema == code_schema:
    ...
    if vault_schema < code_schema:
        print(
            f"Status                 : MIGRATION PENDING "
            f"(v{vault_schema} -> v{code_schema})."
        )
        print(
            "                         Run distill to migrate "
            "(auto-backs up _sync/ first)."
        )
        return
    print(
        "Status                 : ERROR — vault is NEWER than this code."
    )
...
def hive_push(
    config: dict, notes: list[VaultNote] | None = None
) -> int:
    ...
    for note in notes:
        ...
        value = json.dumps({
            ...
            "content": note.clean_body(
                strip_links=True, drop_h1=True, drop_related=True
            ),
...
def hive_pull(config: dict) -> int:
    ...
    for entry in results:
        try:
            note_data = json.loads(entry["value"]) if isinstance(
                entry.get("value"), str
        ...
        if local_path is None:
            ...
            type_dir = vault_root / (
                note_type + "s" if not note_type.endswith("s") else note_type
            )
...
# ---------------------------------------------------------------------------
# Distill runner (shared by CLI command and standalone script)
# ---------------------------------------------------------------------------
...
def run_distill(
    ...
    """Run the distillation. Returns exit code (0 = ok)."""
    ...
    if expired:
        print(
            f"  Skipping {len(expired)} expired note(s): "
            f"{', '.join(n.name for n in expired)}"
        )
    ...
    if list_only:
        for n in sorted(notes, key=lambda x: (x.tier, x.name)):
            print(
                f"  [{n.tier or 'untagged':20}] {n.name:36} type={n.note_type}"
            )
        return 0
    ...
    if graph_only:
        ...
        print(
            f"  God nodes (degree > threshold): {len(graph['god_nodes'])}"
        )
    ...
    cc = targets.get("core_context", {})
    if cc.get("enabled"):
        core_out = sync_core_context(
            active_notes, cc, eager_tiers, strip_links, dry
        )
    ...
    if pa.get("enabled"):
        sync_python_agents(
            active_notes, pa, exclude_tags, vault_only_types, dry
        )
    ...
    if core_out:
        ...
        if cc.get("opencode_config"):
            update_opencode_instructions(
                Path(cc["opencode_config"]), core_out, dry
            )
...
def main() -> None:
    ...
    ap.add_argument(
        "--config",
        default=str(
            Path(__file__).resolve().parent.parent.parent / "cortex.yaml"
        ),
        help=(
            "Path to config file "
            "(default: cortex.yaml next to this script)"
        ),
    )
    ap.add_argument(
        "--hive-push",
    ...
    ap.add_argument(
        "--graph",
        action="store_true",
        help="Output wiki-link graph to distilled/graph.json and exit",
    )
    args = ap.parse_args()
