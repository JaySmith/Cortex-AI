#! /usr/bin/env python3
"""
cortex.mcp.upsert — Upsert the `cortex` MCP entry into an opencode config.
...
Usage:
  cortex.mcp.upsert --mcp-entry <index.js> --memory-json <memory.json> \
...
def upsert(text: str, entry: dict) -> str:
    ...
    if _object_is_empty(text, mcp_val_start):
        insertion = (
            ...
            f"{_indent_of(text, mcp_key_start)}"
        )
    else:
        insertion = f'\n{inner_indent}"cortex": {reindented},'
...
def run(
    ...
    """Run the upsert operation. Returns exit code (0 = ok, 1 = error)."""
    cfg_dir = Path(config_dir).expanduser() if config_dir else (
        Path.home() / ".config" / "opencode"
    )
    target, exists = resolve_target(cfg_dir)
...
def main() -> int:
    """CLI entry point for cortex-mcp-upsert.py (standalone script compat)."""
    ...
    return run(
        ...
        distill_script=args.distill_script,
        distill_python=args.distill_python,
