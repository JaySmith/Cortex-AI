#!/usr/bin/env python3
"""
cortex-uninstall.py — Revert a machine to its pre-Cortex state.
...
Usage:
  python3 cortex-uninstall.py --vault /path/to/vault --latest          # dry-run plan
  python3 cortex-uninstall.py --vault /path/to/vault --latest --apply  # do it
  python3 cortex-uninstall.py --vault /path/to/vault --backup 20260710-152233-setup --apply
  python3 cortex-uninstall.py --vault /path/to/vault --latest --apply --purge
...
def resolve_manifests(vault: Path, latest: bool, backup: str | None) -> list[Path]:
    ...
    if latest:
        return [manifests[-1]]
    # Default: undo everything, newest first (so restores layer correctly).
...
def delete_created(action: dict, apply: bool) -> None:
    ...
    if path.is_dir():
        ...
        path.unlink()
        # Clean up a now-empty parent skill dir (cortex-ai/).
        parent = path.parent
...
def process_manifest(manifest_path: Path, apply: bool) -> None:
    ...
    # Restore modified/backup first, then delete created.
    for a in actions:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Revert Cortex changes using install/import manifests. "
                    "Keeps your vault notes.")
    ap.add_argument("--vault", required=True, help="Vault root Cortex was installed against")
    ...
    manifests = resolve_manifests(vault, args.latest, args.backup)
    ...
    for m in manifests:
        process_manifest(m, args.apply)

    if args.purge:
        purge_distilled(vault, args.apply)
